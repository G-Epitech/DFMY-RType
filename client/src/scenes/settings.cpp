...
void SceneSettings::OnCreate() {
  ...
  CreateMainEntity();
  CreateColorBlindnessSetting(600, 350);
}
...
void SceneSettings::CreateTitle() const {
  ...
  registry_->AddComponent<Position>(
          title, {context_.windowManager->width_ / 2, 0 + 150, HorizontalAlign::kCenter,
                  VerticalAlign::kCenter});
  registry_->AddComponent<Drawable>(
          title, {Text{"Settings", "main", 50}, WindowManager::View::HUD});
}
...
void SceneSettings::CreateBackButton() const {
  ...
  registry_->AddComponent<Drawable>(
          exit_button, {Text{"Back", "main", 20}, WindowManager::View::HUD});
  registry_->AddComponent<OnMousePressed>(
          exit_button, OnMousePressed{
...
void SceneSettings::CreateFullscreenButton(const float &x, const float &y) const {
  ...
  registry_->AddComponent<Drawable>(
          fullscreen_button,
          {Texture{"menu", 4, window_style == sf::Style::Default ? disable : active},
...
void SceneSettings::CreateFullscreenLabel(const float &x, const float &y) const {
  ...
  registry_->AddComponent<Drawable>(
          fullscreen_label, {Text{"Fullscreen", "main", 20}, WindowManager::View::HUD});
}
...
void SceneSettings::CreateDisableSoundsLabel(const float& x, const float& y) const {
  ...
  registry_->AddComponent<Position>(
      volume_label, {x, y, HorizontalAlign::kLeft, VerticalAlign::kCenter});
  registry_->AddComponent<Drawable>(
      volume_label, {Text{"Disable sounds", "main", 20}, WindowManager::View::HUD});
}
  ...
  void SceneSettings::CreateColorBlindnessLabel(const float &x, const float &y) {
    ...
    registry_->AddComponent<Position>(
            ...
            {x, y, HorizontalAlign::kLeft, VerticalAlign::kCenter});
    registry_->AddComponent<Drawable>(
            label, {Text{"Color settings", "main", 20}, WindowManager::View::HUD});
  }

  void SceneSettings::CreateColorBlindnessRadioOption(const std::string &label, const std::string &value,
                                                      const zygarde::core::types::Vector2f &position) {
    auto label_entity = registry_->SpawnEntity();
    auto radio_entity = registry_->SpawnEntity();
    auto on_mouse_pressed = [this, radio_entity, value](const sf::Mouse::Button &button, const sf::Vector2f &pos,
                                                        const events::MouseEventTarget &target) {
        if (button == sf::Mouse::Button::Left) {
    ...
    registry_->AddComponent<Position>(radio_entity,
                                      {position.x, position.y, HorizontalAlign::kLeft, VerticalAlign::kCenter});
    registry_->AddComponent<Drawable>(radio_entity,
                                      {Texture{"menu", 2.5}, WindowManager::View::HUD});

    registry_->AddComponent<Radio>(radio_entity, {
            .id = "color_blindness",
            .value = value
    });
    registry_->AddComponent<OnMousePressed>(radio_entity, {
            .strategy = events::MouseEventTarget::kLocalTarget,
            .handler = on_mouse_pressed
    });
    registry_->AddComponent<Position>(label_entity,
                                      {position.x + 50, position.y, HorizontalAlign::kLeft, VerticalAlign::kCenter});
    registry_->AddComponent<Drawable>(label_entity, {Text{label, "main", 13}, WindowManager::View::HUD});
    registry_->AddComponent<OnMousePressed>(label_entity, {
            .strategy = events::MouseEventTarget::kLocalTarget,
            .handler = on_mouse_pressed
    });
  }
