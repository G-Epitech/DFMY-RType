...
:::info
The ECS SDK is available in the `zygrade::ECS` namespace.
:::
...
class LoggingSystem final : public ASystem<position, velocity> {
 public:
  ...
  void Run(std::shared_ptr<Registry> r, sparse_array<position>& positions,
           sparse_array<velocity>& velocities) override {
    for (size_t i = 0; i < positions.size(); ++i) {
      const auto& [x, y] = positions[i].value();
      std::cout << i << ": Position = { " << x << ", " << y << " }" << std::endl;
...
int main() {
  auto registry = zygarde::ECS::Registry::create();
