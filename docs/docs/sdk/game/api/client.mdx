...
Next, you need to connect to the server with the `Connect` method.
...
```cpp
payload::JoinLobby joinPayload = {
  .lobbyId = 1
};

auto success = client->JoinLobby(joinPayload);
if (!success)
...
:::note
The first `Connect` method have a timeout of 5 seconds.\
The `JoinLobby` method have a timeout of 10 seconds.
:::
