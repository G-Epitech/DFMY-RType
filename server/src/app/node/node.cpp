...
int rtype::server::Node::Node::Run() {
  api_.Start([this](std::uint64_t id, std::size_t maxPlayers, std::size_t difficulty) {
    return this->CreateNewRoom(id, maxPlayers, difficulty);
  });
  api_.Join();
  return EXIT_SUCCESS;
}
...
bool rtype::server::Node::CreateNewRoom(std::uint64_t roomId, std::size_t maxPlayers,
  ...
  boost::process::child roomProcess(ctx_.props.binaryPath, "--name", ctx_.name,
                                    "--port", std::to_string(ctx_.port),
                                    "--ticks", std::to_string(60),
                                    "--token", ctx_.props.token,
