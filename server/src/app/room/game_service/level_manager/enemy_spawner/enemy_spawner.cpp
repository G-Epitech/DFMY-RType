...
Entity EnemySpawner::SpawnEnemy(const std::string& enemy_archetype_name) {
  ...
  if (scriptPool.has_value() && scriptPool.value()) {
    auto movementScript = scriptPool.value()->GetScript<scripts::ZigZagMovementScript>();
    movementScript->SetBasePosition(position);
    auto pataScript = scriptPool.value()->GetScript<scripts::PataScript>();
    pataScript->SetDifficultyData(difficulty_.multipliers.enemy);
  }
