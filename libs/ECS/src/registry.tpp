...
sparse_array<Component> &registry::register_component() {
  ...
  remove_functions.push_back([type_idx](registry &reg, const Entity &e) {
...
sparse_array<Component> &registry::get_components() {
  ...
  return std::any_cast<sparse_array<Component> &>(_components_arrays.at(type_idx));
}
...
typename sparse_array<Component>::reference_type registry::emplace_component(Entity const &to,
  ...
  return components.emplace_at(static_cast<size_t>(to), std::forward<Params>(params)...);
}
